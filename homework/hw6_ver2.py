...
with fiona.open('china_cities_prj.shp', encoding='utf-8', driver='ESRI Shapefile', schema=schema) as shp_data:
    ...
    if submit_button:  # 用户点击查询按钮
        ...
        buffer=Point(selected_city['geometry']['coordinates']).buffer((distance_km * 1000))  # 建立缓冲区
        for city in shp_data:
            if buffer.contains(Point(city['geometry']['coordinates'])) and city['properties']['城市名'] != selected_city_name:
                nearby_city[f'{city['properties']['城市名']}'] = Point(selected_city['geometry']['coordinates']).distance(Point(city['geometry']['coordinates']))
